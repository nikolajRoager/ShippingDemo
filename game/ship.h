...
public:
    ...
    void destroy() {health_=0;}

    Ship(const std::string &className,Side side_, int health,double maxSpeed,double cruiseSpeed,double mass, double length,glm::dvec2 position, double heading,std::shared_ptr<const TexWrap> texture,std::shared_ptr<const TexWrap> markerTexture,std::shared_ptr<const TexWrap> targetTexture,std::shared_ptr<const TexWrap> velocityTexture,SDL_Renderer *renderer,TTF_Font* smallFont);
